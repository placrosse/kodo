...
/// @file test_backward_coefficient_iterator.cpp Unit test
///       for the backward_coefficient_iterator

#include <cstdint>
#include <gtest/gtest.h>

#include <kodo/backward_coefficient_iterator.hpp>
...
namespace kodo
{
    ...
    template<class FieldType>
    class dummy_stack :
        public backward_coefficient_iterator<
...
TEST(TestBackwardCoefficientIterator, binary)
{
    ...
    auto from = stack_type::iterator_type::min_index(0, stack.symbols()-1);
    auto to = stack_type::iterator_type::max_index(0, stack.symbols()-1);

    EXPECT_EQ(from, stack.symbols() - 1);
    EXPECT_EQ(to, 0);
    ...
    // it = stack.coefficient_iterator(&buffer[0], 1);

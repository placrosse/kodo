...
/// @todo We have two anonymous namespaces here, that looks a bit weird.
    ...
    namespace
    {
        ...
        struct api_coder
        {
            ...
            void run_general_api()
            {
                ...
                EXPECT_EQ(coder->symbol_value(index),
                          proxy->symbol_value(index));

                EXPECT_EQ(coder->symbols_available(),
                          proxy->symbols_available());

                EXPECT_EQ(coder->symbols_initialized(),
                          proxy->symbols_initialized());
                ...
                std::vector<value_type> dest_coder(
                    coder->symbol_length(), 'b');

                std::vector<value_type> dest_proxy(
                    coder->symbol_length(), 'b');
            ...
            void run_encoder_api()
            {
                ...
                std::vector<uint8_t> coefficients(
                    coder->coefficient_vector_size(), '3');
                ...
                {
                    EXPECT_EQ(coder->is_symbol_pivot(i),
                              proxy->is_symbol_pivot(i));
                }
            ...
            void run_decoder_api()
            {
                ...
                std::vector<uint8_t> coefficients_coder(
                    coder->coefficient_vector_size(), '3');

                std::vector<uint8_t> coefficients_proxy(
                    coder->coefficient_vector_size(), '3');

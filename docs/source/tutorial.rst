...
the symbol size is typically much larger, e.g., in the order of several
megabytes.
...
limited example, we extend this in a step by step manner to finally end up
...
smart-pointer is one which behaves just like a normal pointer, but
...
happens when all symbols have been decoded.

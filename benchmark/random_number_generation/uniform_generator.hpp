...
namespace kodo
{
    ...
    template<class RandomNumberGenerator, class SuperCoder, class ValueType>
    ...
    using factory =
        basic_factory<new_uniform_generator<RandomNumberGenerator, SuperCoder>>;
    ...
    private:
        uint32_t pos_next_ran_number = 0;
    ...
    public:
        ...
        void initialize(Factory& the_factory)
        {
            ...
            if(SuperCoder::coeffient_vector_size() >= 4)
            {
    ...
    public:
        ...
        void generate(uint8_t *coefficients)
        {
            ...
            if(!vector_size_3_or_less)
            {
                ...
                uint32_t array_lenght = 2 * size;

                for(uint32_t i = 0; i < size; ++i)
                {
                    coefficients[i] = random_numbers[(pos_next_ran_number + i) % array_lenght];
                }
                ...
                random_numbers[pos_next_ran_number] = m_distribution(m_random_generator);
                pos_next_ran_number = (pos_next_ran_number + 1) % array_lenght;
            }
            else
            {
                for(uint32_t i = 0; i < size; ++i)
    ...
    private:
        ...
        std::vector<ValueType> random_numbers;

        //Generates 2 * MAX_SYMBOL_SIZE of random numbers. These are the numbers
        //that will be used as coefficents.
        template<class Factory>
        void generate_first_ran_numbers(Factory& the_factory)
        {
            uint32_t array_size =
                (the_factory.max_coefficient_vector_size()*2)/sizeof(ValueType);
            random_numbers.resize(array_size);
            for(uint32_t i = 0; i <  random_numbers.size(); ++i)
            {
                random_numbers[i] = m_value_distribution(m_random_generator);
            }

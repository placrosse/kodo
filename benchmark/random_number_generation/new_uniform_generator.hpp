...
namespace kodo
{
    ...
    template<class RandomNumberGenerator, class SuperCoder>
    ...
    public:
        ...
        void generate(uint8_t *coefficients)
        {
            ...
            for(uint32_t i = 0; i < size; ++i)
            {
                ...
                // if(i < 3)
                // {
                    //printf("The number is %u and taken from position %u\n", coefficients[i], pos_next_ran_number + i);
                // }
            }
    ...
    private:
        ...
        {
            uint32_t i;
            random_numbers.resize(the_factory.max_coefficient_vector_size() * 2);
            for(i = 0; i <  random_numbers.size(); ++i)
            {
                random_numbers[i] = m_distribution(m_random_generator);
                //printf("%u\n", random_numbers[i]);
            }

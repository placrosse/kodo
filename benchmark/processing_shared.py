...
def plot_sparse_group(group, y_value="mean", title="", group_value = 'average_nonzero_symbols'):
    """
    ...
    p = group.pivot_table(y_value,  rows='symbols', cols=['benchmark',group_value]).plot()
    set_markers(p)
    p.set_title(title, ha = "left", position = (.0,1.03), fontsize = "medium")
